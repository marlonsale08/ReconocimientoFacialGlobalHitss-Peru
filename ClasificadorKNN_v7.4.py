    ...
    while True:
        ...
        if process_this_frame:
            ...
            for face_encoding in face_encodings:
                ...
                print(predictions)
                if predictions:
                    faceID = Known_face_personalHitss.index(str(predictions[i][0])) if (predictions[i][0]!='Desconocido') else -1
                    print("FACE ID: ",faceID)
                    name=str(predictions[i][0])
                    if faceID != -1: 
                        if Known_face_bandera[faceID] == 0:
                            Known_face_bandera[faceID] = 1
                            face_names.append(str(predictions[i][0]))
                            #print(Known_face_bandera[faceID])
                            voiceSpeech(str(predictions[i][0]))
